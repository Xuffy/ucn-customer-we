<template>
  <el-row class="btns" :style="{ width: '100%', paddingLeft: hideMune ? '65px' : '195px' }">
    <div v-if="!edit">
      ...
      <el-button size="mini" type="primary" v-if="pageTypeCurr!='loadingListDetail'&&(logisticsStatus==2)" @click.stop="$emit('switchEdit','confirm')">{{ $i.logistic.confirm }}</el-button>
      ...
      <el-button size="mini" type="primary" v-if="pageTypeCurr=='loadingListDetail'&&logisticsStatus!=4" @click.stop="$emit('switchEdit','cancelLoadingList')">{{ $i.logistic.cancel}}</el-button>
