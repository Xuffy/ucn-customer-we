    ...
    <v-table
      ...
      <template slot="header">
        <div class="btns">
          <el-button
            @click="download"
            v-authorize="'PRODUCT:COMPARE_ARCHIVE:DOWNLOAD'">
            {{$i.product.download+' ('+downloadBtnInfo+')'}}</el-button>
          <el-button
            @click="recover"
            :disabled="disableDelete"
            :loading="disableClickDeleteBtn"
            v-authorize="'PRODUCT:COMPARE_ARCHIVE:RECOVER'">
            {{$i.product.recover}}</el-button>
          <select-search
            v-model="searchValue"
  ...
  export default {
    ...
    methods: {
      ...
      recover(){
        ...
        this.$ajax.post(this.$apis.RECOVER_COMPARE,params).then(res=>{
          this.getList();
        }).finally(()=>{
