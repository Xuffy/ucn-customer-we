var orderType = sessionStorage.getItem('tailorx-order-type');
sessionStorage.setItem('orderEditUrl', window.location.href);
var orderEdit = {
	init: function() {
		...
		orderEdit.flowCode = getval.split("=")[1].split('&')[1].split(',')[0];
		this.initOrder();
	...
	initOrder: function() {
		$.ajax({
			type: 'get',
			url: headUrl.headUrl + "/v3/store/order/getOrderInfoDetail",
			data: {
			...
			success: function(res) {
				if(res.success) {
					...
					//工艺单
					if(res.data.orderWorkUpdateStatus == 0 || res.data.orderWorkUpdateStatus == 1) {
					...
					$('#createDateStr').text(res.data.createDateStr);
					$('#deposit').text(res.data.deposit);
					$('#orderName').text((!res.data.genderTagName ? '--' : res.data.genderTagName) + (!res.data.typeTagName ? '--' : '--' + res.data.typeTagName) + (!res.data.seasonTagName ? '' : '--' + res.data.seasonTagName));
	...
	initOrderInfo: function(flowCode) {
		$.ajax({
			type: "get",
			url: headUrl.headUrl + "/v3/store/order/getOrderInfoLog",
			data: {
				orderNo: orderEdit.orderNo,
				flowCode: flowCode
			},
					...
					$.each(res.data, function(i, value) {
						...
						if(value.operation == 2 && value.status == 0) {
							switch(value.tacheId) {
								...
								case 8:
									{
										if(value.orderWorkUpdateOrCreate == "1") {
	...
	orderCreationDetail: function(status) {
		$('.fasterEn-menu').on('click', function() {
			if(status == 0) {
				sessionStorage.setItem('tailorx-measurebody-menu', 1);
				sessionStorage.setItem('tailorx-measurebody-orderNo', orderEdit.orderNo);
				window.location.href = 'orderCreatDetail.html';
			} else if(status == 1) {
				sessionStorage.setItem('tailorx-measurebody-menu', 2);
				sessionStorage.setItem('tailorx-measurebody-orderNo', orderEdit.orderNo);
				window.location.href = 'orderCreation.html';
			}
		})
	},
	//快捷入口物流信息
	...
	creatFun: function(tacheId) {
		sessionStorage.setItem('tailorx-measurebody-menu', 0);
		sessionStorage.setItem('tailorx-measurebody-orderNo', orderEdit.orderNo);
		sessionStorage.setItem('tailorx-measurebody-tacheId', tacheId);
		window.location.href = 'orderCreation.html';
	},
	//量体数据
	...
	sendFactorySubmit: function() {
		...
		$.ajax({
			type: "POST",
			url: headUrl.headUrl + "/v3/store/order/taskOrderToFactory",
			async: true,
			dataType: "json",
			data: {
				orderNo: orderEdit.orderNo,
	...
	checkAcceptFun: function() {
		$('#checkAcceptSubmit').on('click', function() {
			...
			$.ajax({
				type: "POST",
				url: headUrl.headUrl + "/v3/store/order/checkTaskOrder",
				dataType: 'json',
				data: {
					orderNo: orderEdit.orderNo,
					remark: passTextarea,
	...
	storeSendFactorySubmit: function() {
		...
		$.ajax({
			type: "POST",
			url: headUrl.headUrl + "/v3/store/order/storeDeliverFactory",
			dataType: 'json',
			data: {
				orderNo: orderEdit.orderNo,
	...
	goodsReceipt: function(reStatus) {
		layer.confirm('确定收货？', {
			...
			$.ajax({
				type: "POST",
				url: headUrl.headUrl + "/v3/store/order/storeReceiptFitting",
				async: true,
				data: {
					orderNo: orderEdit.orderNo,
					isFitting: reStatus
